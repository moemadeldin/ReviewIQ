...
final readonly class GroqReviewService implements AIReviewer
{
    ...
    private function repairJson(string $json): string
    {
        ...
        $json = preg_replace('/\\\\?"/', '"', (string) $json);

        $json = preg_replace('/(?<=[\s{,}])(\w+)"(?=\s*:)/', '"$1"', (string) $json);

        $json = preg_replace('/(?<=[\s{,}])(\w+)\s+"(?!\s*[:}\]\)])/', '"$1": "', (string) $json);

        $json = preg_replace('/([{,])\s*"(\w+)"?\s*([\[\{])/', '$1"$2": $3', (string) $json);

        $json = preg_replace('/:\s*(\w+)"(?=\s*[,}\]])/', ': "$1"', (string) $json);

        $json = preg_replace('/(?<=[\s{,}])(\b\w+\b)(?=\s*:)/', '"$1"', (string) $json);

        $json = preg_replace('/:\s*,/', ': null,', (string) $json);
        $json = preg_replace('/:\s*}/', ': null}', (string) $json);

        return preg_replace('/,\s*([}\]])/', '$1', (string) $json);
    }
